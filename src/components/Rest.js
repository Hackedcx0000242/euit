...
alert("Pls login Or Create Account");
  ...
  let getRestaurantDetails = async () => {
    let url = `http://localhost:3089/api/get-restaurant-details-by-id/${id}`;
  ...
  let getMenuList =async() =>{
    let url = `http://localhost:3089/api/get-menu-items-by-restaurant-id/${id}`;
    let response = await fetch(url, { method: "GET" });
    let data = await response.json();
  ...
  let makePayment =async() => {
    let url= `http://localhost:3089/api/create-orderId`;
    ...
    var options = {
      ...
      handler: async function (response) {
        ...
        let url= `http://localhost:3089/api/verify-payment`;
          ...
          <div className="row">
            ...
            <div className="col-12 d-md-none d-lg-none d-block mt-4">
              <div className="ollll position-relative">
                ...
                <button
                  className="btn btn-outline-light   Gallery position-absolute btn-gallery"
                  data-bs-toggle="modal"
                  data-bs-target="#slideShow"
